...
import CronogramaCalendar from "./Cronograma/CronogramaCalendar.jsx";
...
const Cronograma = ({ user }) => {
  ...
  const columnsForGrid = useMemo(() => {
    return COLUMNAS_VISIBLES.map((headerKey) => {
      ...
      if (headerKey === "Nombre del curso") {
        flex = 2.5;
        minWidth = 250;
      }
      if (headerKey === "Ministerio" || headerKey === "Area") {
        flex = 1.5;
        minWidth = 150;
      }
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        ...
        if (!Array.isArray(rawDataCronograma))
        ...
        const dataObjs = rawDataCronograma.map((instance, idx) => {
          ...
          const obj = {
            ...
            "Fecha inicio de inscripción": formatValue(
              instance.fecha_inicio_inscripcion,
            ),
            "Fecha fin de inscripción": formatValue(
              instance.fecha_fin_inscripcion,
            ),
            Cupo: formatValue(instance.cupo),
            "Cantidad de horas": formatValue(instance.cantidad_horas),
            "Publica PCC":
              instance.es_publicada_portal_cc ?? detalle.publica_pcc,
            "Es Autogestionado":
              instance.es_autogestionado ?? detalle.es_autogestionado,
            ...
            "Restricciones por Departamento":
              instance.detalle_restricciones_por_departamento || [],
            "Restricciones por Correlatividad":
              instance.detalle_restricciones_por_correlatividad || [],
            "Tiene restricción por edad":
              instance.tiene_restriccion_edad ?? detalle.tiene_restriccion_edad,
  ...
  useEffect(() => {
    ...
    let relevantCourses =
      ministerioFilter === "all"
        ? cursosData
        : cursosData.filter((c) => c["Ministerio"] === ministerioFilter);
    const areasSet = new Set(
      relevantCourses.map((c) => c["Area"]).filter(Boolean),
    );
  ...
  useEffect(() => {
    ...
    if (nombreFilter.trim()) {
      ...
      data = data.filter((c) =>
        c["Nombre del curso"]?.toLowerCase().includes(term),
      );
    }
    if (monthFilter !== "all") {
    ...
    try {
      ...
      const dataParaExcel = filteredData.map((item) => {
        ...
        return {
          ...
          "Publica en Portal": formatBooleanToSiNo(
            raw.es_publicada_portal_cc ?? det.publica_pcc,
          ),
          "Es Autogestionado": formatBooleanToSiNo(
            raw.es_autogestionado ?? det.es_autogestionado,
          ),
          ...
          "Tiene restricción edad": formatBooleanToSiNo(
            raw.tiene_restriccion_edad ?? det.tiene_restriccion_edad,
          ),
      ...
      const COLUMNAS_EXCEL = [
        ...
        "Tiene restricción edad",
        "Restricción Edad Desde",
        "Restricción Edad Hasta",
        "Comentario",
        "Asignado CUIL",
        "Asignado Nombre",
        "Asignado Mail",
  ...
  const handleDescargarExcelConModal = useCallback(
    async ({ plataforma, incluirCancelados }) => {
      if (!filteredData.length) return;
      try {
        ...
        if (plataforma !== "ALL") {
          dataToProcess = dataToProcess.filter((item) => {
            const plat = String(item._raw?.plataforma_dictado || "")
              .toUpperCase()
              .trim();
        ...
        if (!incluirCancelados) {
          dataToProcess = dataToProcess.filter((item) => {
            const estado = String(item._raw?.estado_instancia || "")
              .toUpperCase()
              .trim();
            return estado !== "CANC";
          });
        ...
        const dataParaExcel = dataToProcess.map((item) => {
          ...
          return {
            ...
            "Plataforma de dictado": raw.plataforma_dictado,
            "Tipo de capacitación": raw.tipo_capacitacion,
            "Tiene correlatividad": formatBooleanToSiNo(
              raw.tiene_correlatividad,
            ),
            "Tiene restricción edad": formatBooleanToSiNo(
              raw.tiene_restriccion_edad ?? det.tiene_restriccion_edad,
            ),
  ...
  const handleMinisterioChange = useCallback(
    (e) => setMinisterioFilter(e.target.value),
    [],
  );
  ...
  const handleNombreChange = useCallback(
    (e) => setNombreFilter(e.target.value),
    [],
  );
  const handleYearChange = useCallback(
    (e) => setYearFilter(e.target.value),
    [],
  );
  ...
  const handleToggleActivosFilter = useCallback(
    () => setActivosFilterActive((prev) => !prev),
    [],
  );
  const handleToggleOmitirCancelados = useCallback(
    () => setOmitirCancelados((prev) => !prev),
    [],
  );
  ...
  const renderDetailItem = useCallback(
    (label, value, isBoolean = false) => {
      ...
      if (displayValue === "" || displayValue === "-" || value === null)
        return null;
                ...
                <FormControlLabel
                  ...
                  label={
                    ...
                    >
                      <BlockIcon
                        fontSize="small"
                        color={omitirCancelados ? "secondary" : "action"}
                      />
                      <Typography
                        variant="body2"
                        sx={{
                          fontWeight: omitirCancelados ? 600 : 400,
                          color: omitirCancelados
                            ? "secondary.main"
                            : "text.secondary",
                        }}
                      >
          ...
          {selectedRowData && (
            ...
            >
              ...
              <CardContent sx={{ overflowY: "auto", flexGrow: 1, p: 2 }}>
                <List dense>
                  <Divider sx={{ mb: 1 }}>
                    <Chip label="Detalles del Curso" size="small" />
                  </Divider>
                  {renderDetailItem(
                    "Ministerio",
                    selectedRowData["Ministerio"],
                  )}
                  {renderDetailItem("Área", selectedRowData["Area"])}
                  {renderDetailItem(
                    "Tipo de capacitación",
                    selectedRowData["Tipo de capacitación"],
                  )}
                  {renderDetailItem(
                    "Cantidad de horas",
                    selectedRowData["Cantidad de horas"],
                  )}

                  <Divider sx={{ my: 2 }}>
                    <Chip label="Detalles de la Instancia" size="small" />
                  </Divider>
                  {renderDetailItem(
                    "Estado",
                    selectedRowData["Estado de Instancia"],
                  )}
                  {renderDetailItem("Cupo", selectedRowData["Cupo"])}
                  {renderDetailItem(
                    "Inscriptos",
                    selectedRowData["Cantidad de inscriptos"],
                  )}
                  {renderDetailItem(
                    "Medio de inscripción",
                    selectedRowData["Medio de inscripción"],
                  )}
                  {renderDetailItem(
                    "Plataforma de dictado",
                    selectedRowData["Plataforma de dictado"],
                  )}
                  {renderDetailItem(
                    "Comentario",
                    selectedRowData["Comentario"],
                  )}

                  <Divider sx={{ my: 2 }}>
                    <Chip label="Fechas" size="small" />
                  </Divider>
                  {renderDetailItem(
                    "Inicio de Inscripción",
                    selectedRowData["Fecha inicio de inscripción"],
                  )}
                  {renderDetailItem(
                    "Fin de Inscripción",
                    selectedRowData["Fecha fin de inscripción"],
                  )}
                  {renderDetailItem(
                    "Inicio del Curso",
                    selectedRowData["Fecha inicio del curso"],
                  )}
                  {renderDetailItem(
                    "Fin del Curso",
                    selectedRowData["Fecha fin del curso"],
                  )}

                  <Divider sx={{ my: 2 }}>
                    <Chip label="Configuración y Restricciones" size="small" />
                  </Divider>
                  {renderDetailItem(
                    "Es Autogestionado",
                    selectedRowData["Es Autogestionado"],
                    true,
                  )}
                  {renderDetailItem(
                    "Publicada en Portal",
                    selectedRowData["Publica PCC"],
                    true,
                  )}
                  ...
                  <ListItem sx={{ py: 0.8, px: 0 }}>
                    <ListItemText
                      primary={
                        selectedRowData["Tiene restricción por edad"]
                          ? `Desde ${selectedRowData["Edad Desde"]} hasta ${selectedRowData["Edad Hasta"] || "sin límite"} años`
                          : "Ninguna"
                      }
                  ...
                  <ListItem sx={{ py: 0.8, px: 0 }}>
                    <ListItemText
                      primary={
                        ...
                        >
                          {selectedRowData["Restricciones por Departamento"]
                            .length > 0 ? (
                            selectedRowData[
                              "Restricciones por Departamento"
                            ].map((d) => (
                  ...
                  <ListItem sx={{ py: 0.8, px: 0 }}>
                    <ListItemText
                      primary={
                        ...
                        >
                          {selectedRowData["Restricciones por Correlatividad"]
                            .length > 0 ? (
                            selectedRowData[
                              "Restricciones por Correlatividad"
                            ].map((c) => (
                              <Chip
                                key={c.curso_correlativo}
                                label={
                                  c.detalle_curso_correlativo?.nombre ||
                                  c.curso_correlativo
                                }
                                size="small"
                              />

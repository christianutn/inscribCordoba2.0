...
const VisualizacionMisNotasRefentes = () => {
      ...
      const uniqueAreas = [
        ...new Set(
          response
            .map(
              (item) =>
                item.NotaAutorizacion?.detalle_usuario?.detalle_area?.nombre,
            )
            .filter(Boolean),
        ),
      ];
  ...
  const handleRefreshNotas = () => {
    setAlert({
      open: true,
      message:
        "Nota de autorización cargada correctamente. Actualizando lista...",
      severity: "success",
    });
    fetchUltimoEstadoDeAutorizaciones();
  };
  ...
  const filteredAutorizaciones = useMemo(() => {
    return autorizaciones.filter((item) => {
      ...
      const matchReferente =
        referente === "" || nombreCompleto.includes(referente.toLowerCase());
      ...
      <Container maxWidth={false} sx={{ px: { xs: 2, md: 5 } }}>
        ...
        >
          <Typography
            ...
            color="text.primary"
          >
            Gestionar Notas de Autorización
          </Typography>

          <Button
            variant="contained"
            startIcon={<AddIcon sx={{ fontSize: 18 }} />}
            onClick={() => setModalCargarNotaOpen(true)}
            sx={{
              backgroundColor: "#009EE3",
              color: "#fff",

              // Tipografía consistente con el título
              fontFamily: '"Geogrotesque Sharp", sans-serif',
              fontWeight: 600, // Medium/Bold feel
              letterSpacing: "0.3px",
              ...
              textTransform: "uppercase", // consistente con sistema
                  ...
                  >
                    ...
                    >
                      ...
                      <Typography gutterBottom>
                        ...
                        {nota?.fecha_desde
                          ? dayjs(nota.fecha_desde).format("DD-MM-YYYY")
                          : "-"}
                      </Typography>
                      {isAutorizado && (
                        <Typography>
                          ...
                          {item.fecha_desde
                            ? dayjs(item.fecha_desde).format("DD-MM-YYYY")
                            : "-"}
                        </Typography>
                      )}
                    </Box>
        ...
        <ModalCargarNota
          open={modalCargarNotaOpen}
          onClose={() => setModalCargarNotaOpen(false)}
          onSuccess={handleRefreshNotas}
        />
      </Container>
    </Box>
  );

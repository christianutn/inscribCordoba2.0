...
const modalStyle = {
  ...
  color: 'text.primary', // <-- Esta línea fuerza el texto a color negro del theme
  borderRadius: 2,
  boxShadow: 24,
  p: 0,
  maxHeight: '90vh',
...
const CronogramaAdminReducido = () => {
  ...
  const isFilterActive = useMemo(() =>
    ...
    asignadoFilter.trim() !== '',
    [
      nombreFilter, ministerioFilter, areaFilter,
      monthFilter, activosFilterActive, asignadoFilter
    ]);
  // ---- Renderizado ----
      ...
      <React.Fragment key={label}>
        <ListItem sx={{ py: 0.8, px: 0 }}>
          <ListItemText
            ...
            secondary={label}
            primaryTypographyProps={{ fontWeight: 500, wordBreak: 'break-word', color: '#000' }}
            secondaryTypographyProps={{ fontSize: '0.8rem', color: '#000' }}
          />
    ...
    <div style={{ padding: 20 }}>
      ...
      <Paper elevation={1} sx={{ p: 2, mb: 3 }}> {/* UI de Filtros */}
        <Grid container spacing={2} alignItems="center">
          ...
          <Grid item xs={12} sm={6} md={3}><TextField fullWidth label="Buscar por Nombre/Código del curso" variant="outlined" size="small" value={nombreFilter} onChange={handleNombreChange} disabled={loading && cursosData.length > 0} InputProps={{ startAdornment: (<InputAdornment position="start"><SearchIcon color="action" /></InputAdornment>), }} /></Grid>
        ...
        <Box sx={modalStyle}>
          {selectedRowData && originalInstanciaData && ( // Asegurarse que ambos existan
            <Card sx={{ display: 'flex', flexDirection: 'column', flexGrow: 1, overflow: 'hidden' }}>
              ...
              <CardContent sx={{ overflowY: 'auto', flexGrow: 1, p: 2, color: '#000' }}>
                <List dense>

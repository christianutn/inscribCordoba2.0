...
const DatosDesarrolloModal = ({ open, onClose, onSave, record, auxiliaryData }) => {
    const isEdit = !!record;
    ...
    // Build options for the Autocomplete
    const usuariosOptions = useMemo(() => {
        return (auxiliaryData?.usuarios || []).map(u => {
    ...
    useEffect(() => {
        if (record) {
            setFormData({
                mes: record.mes || '',
                anio: record.anio || currentYear,
                cuil: record.cuil || '',
                lineas_cod_modificadas: record.lineas_cod_modificadas ?? '',
                lineas_cod_eliminadas: record.lineas_cod_eliminadas ?? '',
    ...
    const validate = () => {
        ...
        if (!formData.mes) {
            newErrors.mes = 'El mes es obligatorio';
        }
        ...
        if (!formData.cuil) {
            newErrors.cuil = 'El desarrollador es obligatorio';
        }
